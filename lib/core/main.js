 ...
 */
async function search(query, options = {}, getHtml = false, fetchConfigs = {}) {
    ...
    let url = encodeURI(options.ris && Constants.URLS.W_GOOGLE + 'searchbyimage?image_url=' + query
        || Constants.URLS.GOOGLE + 'search?q=' + query + '&ie=UTF-8&aomd=1'
        + (options.safe && '&safe=active' || '') + '&start=' + options.page);
    if (options.additional_params) {
        url = url + '&' + new URLSearchParams(options.additional_params).toString()
    }
    ...
    const response = await fetch(url, requestConfigs).catch((err) => err);//const response = await Axios.get(url, requestConfigs).catch((err) => {console.dir(err)});
    if (response instanceof Error) {
        throw new Utils.SearchError('Could not execute search', {status_code: response?.status || 0, message: response?.message});
    }
    ...
    const results = {
        ...
        local_businesses      : [],
        html                  : getHtml ? response.data : ''
    };

    let parser = new Parser(await response.text());
    ...
    const local_business = parser.local_business;
    results.local_businesses = local_business;
    parser = null;
