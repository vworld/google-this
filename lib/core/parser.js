...
class Parser {
    ...
    get local_business() {
        const local_business = this.$(Constants.SELECTORS.LOCAL_BUSINESS_NAME).text();
        const local_business_address = this.$(Constants.SELECTORS.LOCAL_BUSINESS_ADDRESS).text();
        let local_business_phone = this.$(Constants.SELECTORS.LOCAL_BUSINESS_PHONE).text() || '';
        let local_business_phone_alternate = this.$(Constants.SELECTORS.LOCAL_BUSINESS_PHONE_ALTERNATE).attr('href') || '';
        if (local_business_phone_alternate && local_business_phone_alternate.length > 0) {
            local_business_phone_alternate = local_business_phone_alternate.replace('tel:', '');
            if (local_business_phone_alternate.replace(/\D/g, '') !== local_business_phone.replace(/\D/g, '')) {
                if (local_business_phone.length > 0) {
                    local_business_phone = `${local_business_phone}, ${local_business_phone_alternate}`;
                } else {
                    local_business_phone = local_business_phone_alternate;
                }
            }
        }

        const local_business_website = this.$(Constants.SELECTORS.LOCAL_BUSINESS_WEBSITE).attr('href') || '';
        return {
            name   : local_business,
            address: local_business_address,
            phone  : local_business_phone,
            website: local_business_website
        }
    }
